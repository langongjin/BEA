...
pso_settings_t *pso_settings_new(int dim, double range_lo, double range_hi) {
    ...
    settings->steps = 150;
    settings->c1 = 1.496; //1.496
    settings->c2 = 1.496; //1.496
    settings->w_max = PSO_INERTIA;
    settings->w_min = 0.3;

    settings->clamp_pos = 1;
    settings->nhood_strategy = PSO_NHOOD_RING; // 1
    settings->nhood_size = 6;
    settings->w_strategy = PSO_W_LIN_DEC; // 1
	       ...
	       pso_result_t *solution, pso_settings_t *settings)
{
    ...
    double w = PSO_INERTIA; // weight=0.7298 coupled with c1=c2=1.496
    ...
    // INITIALIZE SOLUTION
    solution->error = - DBL_MAX; //error= best_fitness - 0 for the test functions in this demo
...
//        std::cout<< "RNG_UNIFORM : " << RNG_UNIFORM() << std::endl;
    ...
    for (step=0; step < settings->steps; step++) {
      ...
      // update current step
        ...
        for (i=0; i < settings->size; i++) {
          ...
          // update particle fitness
          fit[i] = obj_fun(pos[i], settings->dim, obj_fun_params);
//          std::cout << " fitness: " << fit[i] << std::endl;

          std::ofstream outvalue;
          outvalue.open("../value.txt", std::ios::app);
          outvalue << std::fixed << fit[i] << std::endl;
...
//      std::cout << "End_tv_sec: " << timeEndEA.tv_sec
//                << " Start_tv_sec: " << timeStartEA.tv_sec
//                << " End_tv_usec: " << timeEndEA.tv_usec
//                << " Start_tv_usec: " << timeStartEA.tv_usec;
      timeDiffEA/=1000;
