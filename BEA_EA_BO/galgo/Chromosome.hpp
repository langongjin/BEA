...
namespace galgo
{
  ...
  template <typename T> class Chromosome
  {
    ...
    void create(int tmpnumgen,
                float MaxFitPerCluster,
                float MaxIndivPerCluster[]);
...
//    std::vector< double > indiv;
  ...
  /*-------------------------------------------------------------------------------------------------*/
  // create new chromosome
  template <typename T> inline void Chromosome< T >::create(int tmpnumgen,
                                          float MaxFitPerCluster,
                                          float MaxIndivPerCluster[])
  {
    chr.clear();
    ...
    {
      param[j] = MaxIndivPerCluster[j];
      std::cout << " param[" << j << "]: " << param[j];

      T newvalue = (T) MaxIndivPerCluster[j];
      initGene(j, newvalue);
    ...
    std::cout << " MaxFitPerCluster[" << tmpnumgen << "]"
              << MaxFitPerCluster << " ";
  }
  ...
  template <typename T>
  inline void Chromosome<T>::evaluate0gen()
  {
    // computing objective result(s)
    result = ptr->Objective(param);
    // computing sum of all results (in case there is not only one objective functions)
    ...
    // initializing fitness to this total
    fitness = total;
    std::cout << " fitness0gen: " << fitness << std::endl;
//
//    std::ofstream outvalue;
//    outvalue.open("../value.txt", std::ios::app);
//    outvalue << std::fixed << fitness << std::endl;
